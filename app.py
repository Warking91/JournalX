...
.logo-tekst { font-family: 'Syne', Tahoma, Geneva, Verdana, sans-serif;
...
def inicijaliziraj_bazu():
    ...
    if c.fetchone()[0] == 0:
        ...
        c.execute("INSERT INTO programi (ime, opis, link) VALUES ('JournalX klijent', 'Glavni šifrirani program za upravljanje JournalX sistemom. Sadrži sve core funkcionalnosti.', 'https://google.com')")
        conn.commit()
    conn.close()
...
def promijeni_lozinku(username, nova_lozinka):
    conn = sqlite3.connect("journalx_v2.db")
    c = conn.cursor()
    c.execute("UPDATE korisnici SET password=? WHERE username=?", (nova_lozinka, username))
    conn.commit()
    conn.close()
...
def generiraj_novu_lozinku(duljina=10):
    ...
    return ''.join(secrets.choice(znakovi) for _ in range(duljina))
...
# --- KOMPONENTA: Prikaz programa (lijepe kartice) ---
def prikazi_programe_kartice(programi, admin_mode=False):
    ...
    # Inicijalizacija session state za otvorene kartice
    ...
    for p in programi:
        ...
        with col1:
            label = "▲ Sakrij opis" if je_otvoren else "▼ Prikaži opis"
        ...
        with col2:
            st.link_button(f"⬇️ Preuzmi", p_link, use_container_width=True)
...
# 3. Login, Registracija i Zaboravljena Lozinka
def login_i_registracija():
    ...
    if izbor == "Prijava":
        ...
        st.info("🔑 Unesite vaše korisničko ime i email adresu. Ako se podudaraju, dobit ćete novu lozinku.")
        ...
        if st.button("Resetiraj lozinku", use_container_width=True):
            if zab_username and zab_email:
                postoji = provjeri_email_i_username(zab_username, zab_email)
                if postoji:
